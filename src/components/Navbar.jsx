          ...
          >
                  ...
                  {navbarLinks.map((item, index) => (
                    ...
                    >
                      {item.link ? (
                        ...
                        <span className="font-medium cursor-default text-accent-900 text-2xl duration-300 hover:underline decoration-accent-300 decoration-4 underline-offset-[10px] py-12 whitespace-nowrap">
                          {item.name}
                        </span>
                      )}
                      {item.submenu && item.submenu.length > 0 && (
                        ...
                        >
                          {item.submenu.map((subitem, subIndex) => (
                            <li
                              key={subIndex}
                              className={`${isActive(subitem, pathname) ? "scale-110 bg-accent-950 text-accent-900" : ""} relative rounded-sm -skew-x-12 hover:bg-accent-950 hover:scale-110 duration-200 px-3 hover:font-medium hover:shadow-sm drop-shadow-sm font-medium hover:text-white`}
            ...
            <div className="absolute inset-y-0 right-0 flex items-center lg:hidden">
              ...
              >
                ...
                <svg
                  className={`${openMobile ? "hidden" : "block"} h-6 w-6 text-accent-900`}
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth="2.0"
                  stroke="currentColor"
                ...
                <svg
                  className={`${openMobile ? "block" : "hidden"} h-6 w-6 text-accent-900`}
                  fill="none"
                  viewBox="0 0 24 24"
                  strokeWidth="2.0"
                  stroke="currentColor"
            ...
            >
              {item.link ? (
                <a
                  href={item.link}
                  target={`${item.link.includes("http") ? "_blank" : "_self"}`}
                  className="font-normal text-3xl block p-5 duration-300 text-accent-900 border-accent-200 whitespace-nowrap group-last:bg-accent-950 group-last:font-medium group-last:border-2 group-last:mt-5 group-last:py-4 group-last:px-8 group-last:-skew-x-12 group-last:text-center group-last:mx-5"
                >
                  {item.name}
                </a>
              ) : (
                <div className="font-normal relafont-thinfull justify-center flex cursor-pointer text-2xl duration-300  border-accent whitespace-nowrap">
                  ...
                  >
                    <IoIosArrowForward
                      className={`${hoveredIndex === index ? "text-accent-950" : "text-accent-950"} size-5`}
                    />
                  </div>
                </div>
              )}
